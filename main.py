...
score_speedup = 100
game_speed = 8.0
skins = ["cobalt", "gold", "insta", "default", "aqua", "black", "bloody",
...
def run_game(genomes, config):
    ...
    score = 0
    score_speedup = 100
    enemies = [Cactus(width + 300 / random.uniform(0.9, 3), height - 85),
               Cactus(width * 2 + 200 / random.uniform(0.9, 3), height - 85),
               Cactus(width * 3 + 400 / random.uniform(0.9, 3), height - 85)]
    dinosaurs = []
    nets = []
    skins_copy = skins[:]
    names_copy = names[:]
    ...
    # dino = Dino(30, height-170, "insta", "Danil")
    ...
    while True:
        ...
        for i, enemy in enumerate(enemies):
            ...
            for j, dinosaur in enumerate(dinosaurs):
                if dinosaur.hitbox.colliderect(enemy.hitbox):
                    genomes[j][1].fitness -= 10  # lower fitness, fail
                    dinosaurs.pop(j)
        ...
        for i in rem_list:
            ...
            for j, dinosaur in enumerate(dinosaurs):
                genomes[j][1].fitness += 5  # raise fitness for every enemy
        ...
        for i, dinosaur in enumerate(dinosaurs):
            ...
            if output[0] > 0.5 and dinosaur.state is not DinoState.JUMP:
                dinosaur.jump()
                genomes[i][1].fitness -= 1  # every jump lowers fitness (bunnyhop)
        ...
        score_label = score_font.render('Scores: ' + str(math.floor(score)), True, (50, 50, 50))
        ...
        # display name dinos
        for i, dinosaur in enumerate(dinosaurs):
            name_label = dname_font.render(dinosaur.name, True, (170, 238, 187))
            name_label_rect = name_label.get_rect()
            name_label_rect.center = (width - 100, 100 + (i * 25))
